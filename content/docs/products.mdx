---
title: Products
---

A product is a software or service that you want to license and distribute to your customers. If your product has multiple variants, such as different editions or tiers, create one product in Keyforge for each variant.

## Perpetual fallback access

When enabled on a product, timed licenses that have expired will still be valid, but shown as limited. You decide how to limit your product's functionality based on your needs. The status returned by the API for these licenses is `fallbacked`.

<Callout>
  This feature is recommended to be used together with [license
  renewals](/addons/payment/updates?type=renewal), giving your customers a way
  to extend their licenses after expiration.
</Callout>

## Customization

### Support email

A customer support email displayed in purchase emails and the portal. It's optional, but if not provided, customers will be instructed to contact you through your Keyforge account email.

### Purchase note

A note displayed in purchase emails and the portal, usually to guide customers on how to activate their license.

### Custom button

A button displayed in purchase emails and the portal, usually used as a download button. You can customize its text and link.

<Callout>
  Optionally add the license key to the custom button link by using the `{key}`
  placeholder. For example, `https://example.com/download?key={key}`.
</Callout>

## Purchase email example

Here is an example of a customized purchase email sent to customers:

<Callout title="Product image">
  If you're in the **Scale** plan, you can add an image to your product. This
  image is currently only displayed in purchase emails.
</Callout>

<ImageZoom
  src="https://static.keyforge.dev/docs/product-purchase-email.png"
  alt="Purchase email example"
  width={353}
  height={511}
  unoptimized
/>
