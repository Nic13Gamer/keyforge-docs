...
The simplest way to get and store the first license token on a device is after activating a license. The SDK provides `activateLicense`.
...
Here is an example of how the license token verification workflow could look like in an application using the client SDK:

```mermaid
flowchart TD
    Start([App startup]) --> Check{Token stored<br/>on device?}
    Check -->|Yes| Validate["validateAndRefreshToken()"]
    Check -->|No| Activate([Ask user to activate license])
    Validate -->|Valid| StoreNew[Store refreshed token]
    Validate -->|Expired but was valid| AskNet([Ask user to connect<br/>to the internet])
    Validate -->|Invalid| Activate
    StoreNew --> Grant([Grant access to app])
    Activate --> Call["activateLicense()"]
    Call --> Store[Store token on device]
    Store --> Grant
    ...
    classDef sdk fill:#3b82f61f,stroke:#3b82f6,color:#3b82f6

    class Grant success
    class AskNet warning
    class Activate danger
    class Validate,Call sdk
```
